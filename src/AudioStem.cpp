#include "AudioStem.h"

// We'll use external references to your existing audio objects
extern AudioPlaySdWav sd_wav_player;
extern AudioAmplifier amp_left;
extern AudioAmplifier amp_right;

AudioStem::AudioStem()
: id(0),
  filename(""),
  groupId(0),
  currentVolume(0.0f),
  fadeInDuration(0),
  fadeOutDuration(0),
  fadeInStartTime(0),
  fadeOutStartTime(0)
{
  // Use the existing global audio objects
  player = &sd_wav_player;
  ampLeft = &amp_left;
  ampRight = &amp_right;
}

AudioStem::AudioStem(int id, const String& filename, int groupId)
: id(id),
  filename(filename),
  groupId(groupId),
  currentVolume(0.0f),
  fadeInDuration(0),
  fadeOutDuration(0),
  fadeInStartTime(0),
  fadeOutStartTime(0)
{
  // Use the existing global audio objects
  player = &sd_wav_player;
  ampLeft = &amp_left;
  ampRight = &amp_right;
}

bool AudioStem::isPlaying() const {
  return player->isPlaying();
}

void AudioStem::play() {
  player->play(filename.c_str());
}

void AudioStem::stop() {
  player->stop();
  currentVolume = 0.0f;
  setVolume(0.0f);
}

void AudioStem::setVolume(float volume) {
  currentVolume = constrain(volume, 0.0f, 1.0f);
  ampLeft->gain(currentVolume);
  ampRight->gain(currentVolume);
}

void AudioStem::fadeIn(unsigned long duration) {
  fadeInDuration = duration;
  fadeInStartTime = millis();
  fadeOutDuration = 0; // Cancel any active fade out
  
  // Start at zero volume
  setVolume(0.0f);
  
  // Start playback
  play();
}

void AudioStem::fadeOut(unsigned long duration) {
  // Only start fade out if currently playing
  if (isPlaying()) {
    fadeOutDuration = duration;
    fadeOutStartTime = millis();
    fadeInDuration = 0; // Cancel any active fade in
  }
}

void AudioStem::update() {
  unsigned long currentTime = millis();
  
  // Handle fade in
  if (fadeInDuration > 0) {
    unsigned long elapsed = currentTime - fadeInStartTime;
    
    if (elapsed < fadeInDuration) {
      float progress = (float)elapsed / fadeInDuration;
      setVolume(progress);
    } else {
      // Fade in complete
      setVolume(1.0f);
      fadeInDuration = 0;
    }
  }
  
  // Handle fade out
  if (fadeOutDuration > 0) {
    unsigned long elapsed = currentTime - fadeOutStartTime;
    
    if (elapsed < fadeOutDuration) {
      float progress = 1.0f - ((float)elapsed / fadeOutDuration);
      setVolume(progress);
    } else {
      // Fade out complete, stop playback
      stop();
      fadeOutDuration = 0;
    }
  }
}
